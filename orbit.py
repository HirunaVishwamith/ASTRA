...
def rv_to_coe(mu_km3_s2: float, r_km: np.ndarray, v_km_s: np.ndarray) -> OrbitalElements:
    ...
    if e > 1e-10:
        ...
        # For near-circular, use argument of latitude as "nu"
        if nmag > 1e-12:
...
def propagate_kepler_universal(mu_km3_s2: float, r0_km: np.ndarray, v0_km_s: np.ndarray, dt_s: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    alpha = 2.0 / r0mag - (float(np.dot(v0, v0)) / mu_km3_s2)  # 1/a for elliptic
...
def earth_rotation_rate_rad_s() -> float:
    """WGS-84 Earth rotation rate (rad/s). Useful for ECI->ECEF later."""
    return 7.2921150e-5


def eci_to_ecef(r_eci_km: np.ndarray, t_since_epoch_s: float, theta0_rad: float = 0.0) -> np.ndarray:
    """
    Convert ECI position to ECEF using a simple Earth rotation model:
    ECEF = R3(theta) * ECI, where theta = theta0 + omega*t.

    This is a good first-order approximation for visualization and ground-relative
    behavior; upgrade later to GMST/GAST if needed.
    """
...
def ecef_to_geodetic_spherical(r_ecef_km: np.ndarray) -> Tuple[float, float, float]:
    """
    Spherical-Earth geodetic approximation.
    ...
    alt = rmag  # caller can subtract Earth radius they use
    return lat, lon, alt


def has_line_of_sight(r1_km: np.ndarray, r2_km: np.ndarray, body_radius_km: float, clearance_km: float = 0.0) -> bool:
    """
    True if the segment connecting r1 and r2 does NOT intersect the central body.
    Uses closest-approach distance from origin to the segment.
    """
